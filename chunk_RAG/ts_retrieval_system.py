...
class RetrievalSystem:
    ...
    def get_casetext_from_neo4j(self, case_ids: List[int]) -> List[Dict]:
        """
        Retrieve case_text for the given case ids from Neo4j
        
        Args:
            case_ids: List of case ids
            
        Returns:
            List of dictionaries containing case_text information
        """
        try:
            with self.neo4j_driver.session() as session:
                query = """
                MATCH (c:case_node {case_id: $case_id})
                RETURN c.case_text AS case_text
                """
            ...
            print(f"從 Neo4j 獲取 case_text 時發生錯誤: {str(e)}")
        raise
