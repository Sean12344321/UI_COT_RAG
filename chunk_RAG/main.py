...
load_dotenv()
NEO4J_URI = os.getenv("NEO4J_URI")  # Load NEO4J_URI from environment variables
NEO4J_USER = os.getenv("NEO4J_USER")      # Load Neo4j username from environment variables
NEO4J_PASSWORD = os.getenv("NEO4J_PASSWORD")  # Load Neo4j password from environment variables
...
def retrieval(user_query):
    ...
    for case_id in case_ids:
        with driver.session() as session:
            query = """
            UNWIND $case_id AS case_id
            MATCH (c:case_node {case_id: case_id})
            RETURN c.case_id AS case_id, c.case_text AS case_text
            """
            result = session.run(query, case_id=case_id)
            records.append(result.data()[0]['case_text'])
    return records
...
# print('找到的前5筆Cases: ', retrieval(user_query))
# main()